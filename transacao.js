class Transacao {
  ...
  static transferencia (contaOrigem, contaDestino, idTransacao, dataDeTransacao, valorDaTransferencia) {
    ...
    if (this.contaOrigem.saldo >= this.valorDaTransferencia) {
      ...
      const historicoContaOrigemTransferencia = {
        idTransacao: idTransacao,
        dataDeTransacao: dataDeTransacao,
        valorDaTransferencia: valorDaTransferencia,
        tipo: "pagamento",
      };
      contaOrigem.historico.push(historicoContaOrigemTransferencia);
      

      contaDestino.saldo += valorDaTransferencia;
      const historicoContaDestinoTransferencia = {
        idTransacao: idTransacao,
        dataDeTransacao: dataDeTransacao,
        valorDaTransferencia: valorDaTransferencia,
        tipo: "recebimento",
      };
      contaDestino.historico.push(historicoContaDestinoTransferencia);

      return {mensagem: "Transferência realizado com sucesso!"};
...
/* DEPÓSITO */
  static deposito (contaDestino, idDeposito, dataDoDeposito, valorDoDeposito) {
    ...
    const historicoContaDestinoDeposito = {
      idDeposito: idDeposito,
      dataDoDeposito: dataDoDeposito,
      valorDoDeposito: valorDoDeposito,
      tipo: "recebimento",
    };
    contaDestino.historico.push(historicoContaDestinoDeposito);
...
/* PAGAMENTO SALARIO */
  static pagamentoSalario (contaOrigem, contaDestino, idPagamento, dataDoPagamento, valorDoSalario) {
    ...
    if (contaOrigem != Pessoa && valorDoSalario > 1000) {
      return {
    ...
    if (contaOrigem.saldo >= valorDoSalario) {
      ...
      const historicoContaOrigemPagamentoSalario = {
        idPagamento: idPagamento,
        dataDoPagamento: dataDoPagamento,
        valorDoSalario: valorDoSalario,
        tipo: "pagamento",
      };
      contaOrigem.historico.push(historicoContaOrigemPagamentoSalario);

      const historicoContaDestinoPagamentoSalario = {
        idPagamento: idPagamento,
        dataDoPagamento: dataDoPagamento,
        valorDoSalario: valorDoSalario,
        tipo: "recebimento",
      };
      contaDestino.historico.push(historicoContaDestinoPagamentoSalario);
...
console.log(ChurrosVictor.nomeFantasia); // Deve retornar ChurrosVictor   /* Obs.: acredito que o teste deveria vir: 'Deve retornar Churros do Victor' */
